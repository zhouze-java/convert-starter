package com.easy.convert.util.word
...
class WordConvertUtil(private val wordProperties: WordProperties) {
     ...
     * 通过文件流转换格式
     */
    fun convertFormat(wordInputStream: InputStream, outputSteam: OutputStream, saveFormat: String) {
        val format: Int
        when {
            saveFormat.equals("doc", ignoreCase = true) -> {
                format = SaveFormat.DOC
        ...
        // 加载源文件
        val doc = Document(wordInputStream)
        // 判断是否需要添加水印
        if (wordProperties.watermarks.enable){
            addConfigWatermarks(doc)
        }
        // 保存
        doc.save(outputSteam, format)
    }
